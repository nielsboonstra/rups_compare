...
def explanation_dialog():
    ...
    st.markdown("- Één of meerdere kolommen met programmeerjaartallen, met óf een 'X' in de rijen onder de jaartalkolommen (b.v. een X onder 2024 of 2025), óf als integer getal onder één jaarkolom (bv. een kolom 'Programmeerjaar').")
    st.markdown("ℹ️ _Het maakt niet uit of je Excel één of meerdere werkbladen heeft. Bij > 1 werkbladen zal de webapp je vragen om een werkblad te selecteren._ Daarnaast hoeven bovengenoemde kolommen niet een specifieke kolomnaam te hebben; je kan in een volgende stap zelf de juiste kolommen selecteren.")
    st.divider()
    st.markdown("### Output")
...
if st.session_state['df_old'] is not None:
    st.warning('Let op! Het kan zijn dat de tool hieronder foutmeldingen laat zien vanuit het achterliggende scipt. Controleer in dat geval hieronder of je data goed is ingeladen. Check vooral of de headers op de juiste rij zijn ingelezen.')
